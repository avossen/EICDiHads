...
#    xEdges=[10**(-3+i/3) for i in range(8)]
    ...
    cf=plt.contourf(xMean, etaMean, z, alpha=.75, cmap='jet',locator=ticker.LogLocator(numticks=20,subs=range(1,10)))
...
def plotXVsQ2(data):
    ...
    xEdges=np.logspace(-3,-1,30)

    
    #xEdgesHigh=xEdgesLow[1:]
    q2Edges=np.logspace(-1,4,30)
    #this stores the eta for each hadron in the pair, so we have to flatten this structure
    arQ2=data["Q2"]
#    arQ2=arEta.flatten();
    #get x for the other dimension, now of course we have only have as many x values
    arX=data["x"]
#    arX=np.repeat(arX,2)
    hist2d=np.histogram2d(arX,arQ2,bins=(xEdges,q2Edges))
    #should be done better, namely at mean of log
    xMean=[exp((log(xEdges[i])-log(xEdges[i-1]))/2+log(xEdges[i-1]))  for i in range(len(xEdges)) if(i>0)]
...
def plotXVsEtaPt(data):
    ...
    hist2d=np.histogram2d(arX,arPt,bins=(etaEdges,ptEdges),weights=arX)
    hist2d_unweighted=np.histogram2d(arX,arPt,bins=(etaEdges,ptEdges))
